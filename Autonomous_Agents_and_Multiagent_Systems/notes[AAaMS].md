...
- <u>Utility Function</u>  
  given the set of outcomes (the domain is the set of outcomes) and as codomains the set of real numbers.  
...
- <u>Protocol</u>  
  ...
  The way the negotiation must be conducted cannot be violated in general (example: if you are the seller, you offer should decrease, if you are the buyers, your offer should increase).  

- <u>Strategies</u>  
  policies the single agents will adopt for conducting the negotiation.  
  within the protocol, the agent decides what to do (example: the seller decides yo offer 5 less every time, the buyer to offer 10 more every time).  
...
Now we can start to discuss the theory that will bring us to the final result of axiomatic approach.  
We will list a number of properties we would like the bargains solution to satisfy.  
At the end we will discover that if we want our bargain solution to satisfy all these properties then the bargains solution will have a certain form.  
...
$$
Considering a specific agent $i$.
...
5. <u>Independence of irrelevant alternatives</u>  
   $\forall S' \subseteq S \ \ \ \ \  f(S,d) \in S' \ \ \ \ \ \ \ \ f(S,d)=f(S',D) $        
...
We will now focus on the agreement found by the agent, not on the policy.
...
In case of convex games we can say that fair outcomes are also stable in case of convex games.
...
Let's assume we have $3$ agents: $A=\{a_1,a_2,a_3\}$. What are the possible coalitions?
$$
...
- I've to look at all possible partitions in each subsets (I'm talking  about the inner maximum operator) $\to$ this is not a problem most of the time because listing all possible partition in a subset with respect to the number of partitions of the entire set $A$.
...
$(6) \ \text{delete from $C_i$ coalitions that contain argents in $C_{max}$}$
...
you can have a planning model like STRIPS or like Markov decision problem.
...
$O(<hl,hr>|<L,L>,T_L)=<0.85,0.15> \to$ the tiger is behind the left door. both agents were listening to the left door. Agent 1 has the right perception with probability $0.85$.
...
- Each agents knows only about the constraints that involve its variable/s.
...
1. utils propagation:  
   start from the bottom of the three and report some "util messages".
2. value propagation:  
...
$Sep_i:$ set of other nodes such that if I delete these other nodes, node $i$ and its subtree are independent from the rest of the tree.
...
you system is operating in some context and you'd like it to adapt to the particular contest and change if the contest changes. this is  the generally idea of single agents.  
What changes in multi agent? 

- very big space of states ( the space of joint actions is way larger than the space of single actions)
...
- learnability: how much the actions performed by one agent affect the goal of that agent, or how much the goal of an agent depends on the action that that agent performs.  
  example: 
...
we have a system that through a process of trials and errors tries to perform better in a specific environment. in order for the model to work we need the environment to be episodic.
...
a policy $\pi^i:S \to A^i$ tells, given a state, what are the actions agent $i$ can perform (we are considering the case that policies are deterministic, but it does not need to be the case. policies could be defined as probability distributions over states)
...
Agents receive a negative reward if the bump into each other.
$$
A^i=\{W,E,N,S\}
\\
S=\{(0,0),(0,1),(0,2),...\}
...
this we do not care who reaches his station, but that one of them does.
